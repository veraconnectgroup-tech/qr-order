...
describe("denisTurnDailyDelta", () => {
  ...
  it("counts LLM turns with credit cost", () => {
    ...
    expect(delta.roiImpact.tokens_total).toBeGreaterThan(0);
  });

  it("records waiter deflection on T0 turns", () => {
    const delta = denisTurnDailyDelta({
      orgId: "org-1",
      locationId: "loc-1",
      createdAt: "2026-06-07T12:00:00.000Z",
    ...
    expect(delta.roiImpact.waiter_calls_saved).toBe(1);
  });
});
...
describe("mergeRoiImpactMaps", () => {
  it("merges ROI impact counters", () => {
    const merged = mergeRoiImpactMaps(
      roiImpactDelta("waiter_calls_saved", 3),
      roiImpactDelta("waiter_calls_saved", 2)
    );

    expect(merged.waiter_calls_saved).toBe(5);
  });
});
