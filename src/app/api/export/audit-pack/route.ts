...
import {
  auditPackFilename,
  generateFinanzamtAuditPack,
} from "@/lib/export/audit-pack";
import { parseDsfinvkDateRange } from "@/lib/export/dsfinvk";
import { createAdminClient } from "@/lib/supabase/admin";
import { withRateLimit } from "@/lib/rate-limit";
import { zUuid } from "@/lib/security/zod-fields";

export const GET = withErrorHandler("export-audit-pack-get", async (req, _ctx) => {
  ...
  const zipBuffer = await generateFinanzamtAuditPack(
    staff.org_id,
    locationId,
    parsed.from,
    parsed.to,
  ...
  const filename = auditPackFilename(
    locationRow.name,
    parsed.from,
    parsed.to
  );
  ...
  await auditLog({
    ...
    action: "export",
    entityType: "finanzamt_audit_pack",
    newValue: {
      filename,
      locationId,
      from: parsed.from,
