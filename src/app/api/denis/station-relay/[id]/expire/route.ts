...
import { expireRelayMessage } from "@/lib/denis/stations/station-relay-messages";
import { withStaffRateLimit } from "@/lib/rate-limit";
import { isUuid } from "@/lib/security/sanitize";
import { createAdminClient } from "@/lib/supabase/admin";
import { createServerClient } from "@/lib/supabase/server";

/** Marks an unanswered relay as expired once the asking station has been told nobody replied — stops it from being spoken again. */
export const POST = withErrorHandler(
  "station-relay-expire-post",
  async (req, ctx) => {
    ...
    const ok = await expireRelayMessage(admin, { relayId: relayRow.id });
