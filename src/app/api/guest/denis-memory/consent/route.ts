...
import { grantGuestMemoryConsent } from "@/lib/guest/denis-guest-memory-store";
...
const consentSchema = z.object({
  ...
  scopes: z
    .array(z.enum(["allergies", "favorites", "language"]))
    .min(1)
    .max(3),
...
/** Grant consent and optionally seed from local profile (M17). */
export const POST = withErrorHandler(
  "guest-denis-memory-consent",
  async (req) => {
    ...
    const projection = await grantGuestMemoryConsent(admin, {
