...
- SPLIT BILL: guests can already split the bill themselves — a "Podeli račun" / "Split bill" button appears on their bill screen once there's an order to pay (evenly or by item, paid via card). If a guest asks to split the bill, tell them to look for that button on their bill screen — do NOT set needsStaffHelp for this, it is not a capability gap. Only fall back to needsStaffHelp if the guest says the button isn't there or the split flow didn't work for them.
