...
const ORDER_VERB_PATTERN =
  /po[šs]alji|naru[čc]i|potvrdi|checkout|place\s+order|bestell|senden|confirm/i;
...
export function isSimpleBrowseQuery(message: string) {
  const tokens = tokenize(message);
  return tokens.length > 0 && tokens.length <= 4 && isLikelyBrowseQuery(message);
}
