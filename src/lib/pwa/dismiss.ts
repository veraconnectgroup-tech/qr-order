...
const WAITER_DISMISS_KEY = "qr_order_pwa_dismiss_waiter";
...
export function isWaiterBannerDismissed(): boolean {
  if (typeof localStorage === "undefined") return false;
  try {
    const raw = localStorage.getItem(WAITER_DISMISS_KEY);
    if (!raw) return false;
    const { at } = JSON.parse(raw) as { at: number };
    return Date.now() - at < DISMISS_MS;
  } catch {
...
export function dismissWaiterBanner(): void {
  localStorage.setItem(
    WAITER_DISMISS_KEY,
    JSON.stringify({ at: Date.now() })
  );
