...
export const DEFAULT_PROACTIVE_POLICY: ProactivePolicyManifest = {
  ...
  kindPolicy: {
    ...
    order_delay: service(),
    order_eta_update: service(),
    order_ready: service(),
    order_ready_notify: service(),
    kitchen_busy: service(),
    kitchen_busy_preorder: service(),
    drink_refill: upsell([]),
    drink_with_food: upsell([]),
    round_two: upsell([]),
    happy_hour_upsell: upsell([]),
    drink_pairing: upsell([]),
    waiter_gap: service(),
    attention_handoff: service(),
    scroll_search: service(),
    scroll_category: service(),
    scroll_bottom: service(),
  },
};
