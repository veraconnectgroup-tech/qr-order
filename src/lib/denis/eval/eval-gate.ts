...
export function runFullEvalMetrics() {
  ...
  return {
    ...
    venueSimScore: gate.venueSim.overallScore,
    omniscient: gate.omniscient.passRate,
  };
}
...
export function compareEvalToBaseline(
  ...
  return {
    ...
    venueSimScore: {
      baseline: baseline.venueSimScore,
      current: current.venueSimScore,
      delta: current.venueSimScore - baseline.venueSimScore,
      pass: current.venueSimScore >= EVAL_GATE_THRESHOLDS.venueSimScore,
    },
    omniscient: {
      baseline: baseline.omniscient,
      current: current.omniscient,
      delta: current.omniscient - baseline.omniscient,
      pass: current.omniscient >= EVAL_GATE_THRESHOLDS.omniscient,
    },
  };
}
