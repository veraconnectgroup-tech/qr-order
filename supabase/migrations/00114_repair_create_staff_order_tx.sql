-- Repair: 00082 was marked applied but create_staff_order_tx was missing/mismatched on remote.
-- Drop any stale overloads, then recreate the canonical signature.
...
BEGIN
  FOR r IN
    ...
    FROM pg_proc
    WHERE proname = 'create_staff_order_tx'
...
COMMENT ON FUNCTION create_staff_order_tx(
  UUID, UUID, UUID, UUID, TEXT, BOOLEAN, TEXT, JSONB, JSONB
) IS 'Atomic staff order: order + items + modifiers (session must exist).';

GRANT EXECUTE ON FUNCTION create_staff_order_tx(
  UUID, UUID, UUID, UUID, TEXT, BOOLEAN, TEXT, JSONB, JSONB
) TO service_role;
