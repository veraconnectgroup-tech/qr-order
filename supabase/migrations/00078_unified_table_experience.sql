-- Unified Table Experience: POS inbound, session lifecycle, idempotency.
-- Rollback: drop new objects in reverse dependency order.

-- ===== orders: source + payment expansion =====
...
-- Sync access_state for existing closed sessions
...
-- ===== session_payment_intents (multi-order checkout) =====
...
CREATE POLICY "admin_manage_pos_table_mappings" ON pos_table_mappings
  FOR ALL
...
COMMENT ON TABLE pos_order_links IS 'Idempotency: one Vera order per POS external order id.';
COMMENT ON TABLE pos_table_mappings IS 'POS table key to Vera table_id.';
COMMENT ON TABLE pos_inbound_events IS 'Inbound POS webhook audit log.';
COMMENT ON TABLE session_payment_intents IS 'Session-scoped Stripe payment intents.';
